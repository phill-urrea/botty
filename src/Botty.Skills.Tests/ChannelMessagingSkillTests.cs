...
public class ChannelMessagingSkillTests
{
    [Fact]
    ...
    private static async Task<ChannelMessagingSkill> CreateSkillAsync(
        ...
        FakeConversationRepository? repository = null)
    {
        ...
        var skill = new ChannelMessagingSkill(
            registry ?? new FakeChannelRegistry(),
            provider.GetRequiredService<IServiceScopeFactory>(),
            NullLogger<ChannelMessagingSkill>.Instance);

        await skill.InitializeAsync(new SkillConfiguration
        {
            SkillId = "channel_messaging",
            Values = []
        });
