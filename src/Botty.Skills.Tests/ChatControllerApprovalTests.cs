...
public class ChatControllerApprovalTests
{
    ...
    public async Task Chat_CreatesApprovalTask_ForGmailSend_AndSkipsImmediateExecution()
    {
        ...
        var skillRegistry = new FakeSkillRegistry();
        ...
        Assert.Equal(0, skillRegistry.ExecuteToolCallCount);
    ...
    private sealed class FakeSkillRegistry : ISkillRegistry
    {
        ...
        {
            yield return new FakeSkill("gmail_send_message");
            yield return new FakeSkill("channel_send_message");
        }
        ...
        public Task<SkillResult> ExecuteToolAsync(string toolName, string arguments, CancellationToken ct = default)
        {
            ExecuteToolCallCount++;
            return Task.FromResult(SkillResult.Ok("""{"sent":true}"""));
        }

        public Task<SkillResult> ExecuteToolAsync(
            string toolName,
            string arguments,
            Guid? conversationId,
            Guid? taskId,
    ...
    private sealed class FakeSkill : ISkill
    {
        ...
        public FakeSkill(string toolName)
        {
            _toolName = toolName;
        }
        ...
        public SkillConfigSchema ConfigSchema => new() { SkillId = "gmail", Fields = [] };

        public Task InitializeAsync(SkillConfiguration config, CancellationToken ct = default) => Task.CompletedTask;

        public Task<SkillResult> ExecuteAsync(SkillContext context, CancellationToken ct = default) =>
            Task.FromResult(SkillResult.Ok("{}"));
