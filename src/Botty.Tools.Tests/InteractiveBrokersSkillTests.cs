...
public class InteractiveBrokersToolTests
{
    [Fact]
    ...
    private static InteractiveBrokersTool CreateSkill(HttpMessageHandler handler)
    {
        var factory = new StubHttpClientFactory(handler);
        var client = new IbClient(factory, NullLogger<IbClient>.Instance);
        return new InteractiveBrokersTool(client, NullLogger<InteractiveBrokersTool>.Instance);
    }

    private static async Task InitializeAsync(InteractiveBrokersTool skill)
    {
        await skill.InitializeAsync(new ToolConfiguration
        {
            ToolId = "interactive_brokers",
