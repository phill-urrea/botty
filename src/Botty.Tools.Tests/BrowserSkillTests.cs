...
public class BrowserToolTests
{
    ...
    // --- BrowserTool.OnExecuteAsync error handling tests ---
    ...
    private static BrowserTool CreateSkill()
    {
        var session = new BrowserSession(NullLogger<BrowserSession>.Instance);
        return new BrowserTool(session, NullLogger<BrowserTool>.Instance);
    }

    private static async Task InitializeAsync(BrowserTool skill)
    {
