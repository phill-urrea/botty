...
public class SchedulerToolTests
{
    [Fact]
    public async Task ScheduleCronJob_ParsesSnakeCaseArguments()
    {
        ...
        var skill = new SchedulerTool(provider.GetRequiredService<IServiceScopeFactory>(), NullLogger<SchedulerTool>.Instance);
    ...
    private sealed class FakeSchedulerService : ISchedulerService
    {
        ...
        public Task<IEnumerable<ScheduledTask>> GetAllScheduledTasksAsync(CancellationToken ct = default)
            => Task.FromResult(Enumerable.Empty<ScheduledTask>());
        ...
        public Task RunNowAsync(Guid taskId, CancellationToken ct = default)
            => Task.CompletedTask;
