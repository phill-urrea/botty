...
namespace Botty.Skills.Calendar;

/// <summary>
/// Skill for Google Calendar integration supporting multiple accounts.
/// </summary>
public class GoogleCalendarSkill : BaseSkill
{
    ...
    public GoogleCalendarSkill(
        ILinkedAccountService linkedAccountService,
        ILogger<GoogleCalendarSkill> logger) : base(logger)
    {
    ...
    protected override async Task OnInitializeAsync(CancellationToken ct)
    {
        ...
        {
            Logger.LogWarning("Google Calendar skill: OAuth credentials not configured");
            return;
        }
        ...
        {
            Logger.LogWarning("Google Calendar skill: No linked or legacy accounts configured");
            return;
        }
    ...
    private async Task<SkillResult> ListEventsAsync(string arguments, CancellationToken ct)
    {
        ...
        if (service == null) return SkillResult.Fail("No Google Calendar account configured or specified");
        ...
        return SkillResult.Ok(ToJson(new { events = result ?? [], count = result?.Count ?? 0 }));
    }

    private async Task<SkillResult> GetEventAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<GetEventArgs>(arguments);
        if (args == null || string.IsNullOrEmpty(args.EventId))
            return SkillResult.Fail("Invalid arguments: eventId required");

        var service = GetService(args.Account);
        if (service == null) return SkillResult.Fail("No Google Calendar account configured");
    ...
    private async Task<SkillResult> CreateEventAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<CreateEventArgs>(arguments);
        if (args == null || string.IsNullOrEmpty(args.Summary))
            return SkillResult.Fail("Invalid arguments: summary required");
    ...
    private async Task<SkillResult> UpdateEventAsync(string arguments, CancellationToken ct)
    {
    ...
    private async Task<SkillResult> DeleteEventAsync(string arguments, CancellationToken ct)
    {
        ...
        return SkillResult.Ok(ToJson(new { success = true, eventId = args.EventId }));
    }

    private async Task<SkillResult> ListCalendarsAsync(string arguments, CancellationToken ct)
    {
        ...
        return SkillResult.Ok(ToJson(new { calendars = result ?? [], count = result?.Count ?? 0 }));
    }
