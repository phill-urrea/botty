...
namespace Botty.Skills.Channels;

/// <summary>
/// Exposes channel conversation discovery and outbound messaging tools.
/// </summary>
public class ChannelMessagingSkill : BaseSkill
{
    ...
    public ChannelMessagingSkill(
        IChannelRegistry channelRegistry,
        IServiceScopeFactory scopeFactory,
        ILogger<ChannelMessagingSkill> logger)
        : base(logger)
    {
    ...
    private async Task<SkillResult> ListConversationsAsync(string arguments, CancellationToken ct)
    {
    ...
    private async Task<SkillResult> ListRecentSendersAsync(string arguments, CancellationToken ct)
    {
        using var scope = _scopeFactory.CreateScope();
        var conversationRepository = scope.ServiceProvider.GetRequiredService<IConversationRepository>();
    ...
    private async Task<SkillResult> SendMessageAsync(string arguments, CancellationToken ct)
    {
        ...
        {
            return SkillResult.Fail("Missing required parameters: 'chat_id' and 'message'.");
        }
        ...
        if (channel == null)
        {
            return SkillResult.Fail($"Unknown channel '{channelId}'.");
        }
        ...
        if (!result.Success)
        {
            return SkillResult.Fail($"Failed to send message: {result.Error ?? "unknown error"}");
        }
