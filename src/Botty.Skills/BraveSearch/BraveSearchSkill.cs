...
namespace Botty.Skills.BraveSearch;

/// <summary>
/// Skill for real-time web search using the Brave Search API.
/// </summary>
public class BraveSearchSkill : BaseSkill
{
    ...
    public BraveSearchSkill(
        IHttpClientFactory httpClientFactory,
        ILogger<BraveSearchSkill> logger)
        : base(logger)
    {
    ...
    protected override async Task<SkillResult> OnExecuteAsync(SkillContext context, CancellationToken ct)
    {
        ...
        {
            return SkillResult.Fail($"Unknown tool: {context.ToolName}");
        }

        if (string.IsNullOrWhiteSpace(_apiKey))
        {
            return SkillResult.Fail("Brave Search API key is not configured.");
        }

        var args = ParseArguments<BraveSearchArgs>(context.Arguments);
        if (args == null || string.IsNullOrWhiteSpace(args.Query))
        {
            return SkillResult.Fail("Missing or invalid arguments: 'query' is required.");
        }
        ...
        catch (HttpRequestException ex)
        {
            ...
            return SkillResult.Fail($"Search request failed: {ex.Message}");
        }
        catch (TaskCanceledException)
        {
            return SkillResult.Fail("Search request timed out.");
        }
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            return SkillResult.Fail($"Search API returned {response.StatusCode}. Check API key and quota.");
        }
