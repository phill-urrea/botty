...
namespace Botty.Skills.Scripting;

/// <summary>
/// Skill for creating and executing persistent script-backed tools.
/// </summary>
public partial class ScriptSkill : BaseSkill
{
    ...
    public ScriptSkill(ILogger<ScriptSkill> logger)
        : base(logger)
    {
    }
    ...
    protected override Task<SkillResult> OnExecuteAsync(SkillContext context, CancellationToken ct)
    {
    ...
    private async Task<SkillResult> CreateScriptAsync(string arguments, CancellationToken ct)
    {
        ...
        if (args == null ||
            ...
            args.Content == null)
        {
            return SkillResult.Fail("Invalid arguments. Required: name, description, interpreter, entrypoint, content.");
        }

        var name = args.Name.Trim();
        if (!ScriptNameRegex().IsMatch(name))
        {
            return SkillResult.Fail("Invalid script name. Use snake_case and 3-50 characters.");
        }

        if (ManagementTools.Contains(name))
        {
            return SkillResult.Fail($"Script name '{name}' is reserved.");
        }

        var interpreter = args.Interpreter.Trim();
        if (!AllowedInterpreters.Contains(interpreter))
        {
            return SkillResult.Fail("Unsupported interpreter. Allowed: /bin/bash, bash, python3, node.");
        }
        ...
        try
        {
            ...
            {
                return SkillResult.Fail($"Script '{name}' already exists.");
            }

            var scriptDir = GetScriptDirectory(name);
            if (Directory.Exists(scriptDir))
            {
                return SkillResult.Fail($"Directory for script '{name}' already exists.");
            }
    ...
    private Task<SkillResult> ListScriptsAsync(CancellationToken ct)
    {
        ...
        return Task.FromResult(SkillResult.Ok(ToJson(new
        {
            scripts,
    ...
    private async Task<SkillResult> ReadScriptAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<NamedScriptArgs>(arguments);
        if (args == null || string.IsNullOrWhiteSpace(args.Name))
        {
            return SkillResult.Fail("Invalid arguments. Required: name.");
        }

        var name = args.Name.Trim();
        if (!_scripts.TryGetValue(name, out var manifest))
        {
            return SkillResult.Fail($"Script '{name}' not found.");
        }
        ...
        catch (Exception ex)
        {
            ...
            return SkillResult.Fail($"Failed to read script '{name}': {ex.Message}");
        }
    }

    private async Task<SkillResult> EditScriptAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<EditScriptArgs>(arguments);
        if (args == null || string.IsNullOrWhiteSpace(args.Name) || args.Content == null)
        {
            return SkillResult.Fail("Invalid arguments. Required: name, content.");
        }
        ...
        catch (Exception ex)
        {
            ...
            return SkillResult.Fail($"Failed to edit script '{name}': {ex.Message}");
        }
        finally
        {
            _mutationLock.Release();
    ...
    private async Task<SkillResult> DeleteScriptAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<NamedScriptArgs>(arguments);
        if (args == null || string.IsNullOrWhiteSpace(args.Name))
        {
        ...
        catch (Exception ex)
        {
            ...
            return SkillResult.Fail($"Failed to delete script '{name}': {ex.Message}");
        }
        finally
        {
            _mutationLock.Release();
    ...
    private async Task<SkillResult> ExecuteScriptToolAsync(string toolName, string arguments, CancellationToken ct)
    {
        if (!_scripts.TryGetValue(toolName, out var manifest))
        {
            return SkillResult.Fail($"Unknown tool: {toolName}");
        }
        ...
        {
            return SkillResult.Fail($"Entrypoint file not found for script '{toolName}'.");
        }
    ...
    private async Task ScanScriptsAsync(CancellationToken ct)
    {
        ...
        Logger.LogInformation("Scripting skill scanned {Count} script tools from {Workspace}", _scripts.Count, _workspaceDirectory);
    }
