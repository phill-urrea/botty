...
namespace Botty.Skills.Shell;

/// <summary>
/// Skill for executing shell commands with safety controls.
/// </summary>
public partial class ShellSkill : BaseSkill
{
    ...
    public ShellSkill(ILogger<ShellSkill> logger) : base(logger)
    {
    }
    ...
    protected override Task OnInitializeAsync(CancellationToken ct)
    {
        ...
        Logger.LogInformation("Shell skill initialized. Working directory: {WorkDir}, Sudo allowed: {AllowSudo}", 
    ...
    private async Task<SkillResult> ExecuteCommandAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<ExecuteArgs>(arguments);
        if (args == null || string.IsNullOrEmpty(args.Command))
            return SkillResult.Fail("Invalid arguments: command required");

        // Safety check
        var safetyResult = CheckCommandSafety(args.Command);
        if (!safetyResult.IsSafe)
        {
            return SkillResult.Fail($"Command blocked: {safetyResult.Reason}");
        }
        ...
        catch (Exception ex)
        {
            ...
            return SkillResult.Fail($"Command failed: {ex.Message}");
        }
    }

    private async Task<SkillResult> ExecuteScriptAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<ScriptArgs>(arguments);
        if (args == null || string.IsNullOrEmpty(args.Script))
            return SkillResult.Fail("Invalid arguments: script required");
        ...
        foreach (var line in lines)
        {
            ...
            if (!safetyResult.IsSafe)
            {
                return SkillResult.Fail($"Script blocked at line: {safetyResult.Reason}");
            }
        }
    ...
    private async Task<SkillResult> ReadFileAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<ReadFileArgs>(arguments);
        if (args == null || string.IsNullOrEmpty(args.Path))
            return SkillResult.Fail("Invalid arguments: path required");
        ...
        {
            return SkillResult.Fail($"Failed to read file: {ex.Message}");
        }
    }

    private async Task<SkillResult> WriteFileAsync(string arguments, CancellationToken ct)
    {
        ...
        {
            return SkillResult.Fail($"Failed to write file: {ex.Message}");
        }
    }

    private Task<SkillResult> ListDirectoryAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<ListDirArgs>(arguments);
        if (args == null || string.IsNullOrEmpty(args.Path))
            return Task.FromResult(SkillResult.Fail("Invalid arguments: path required"));
        ...
        {
            return Task.FromResult(SkillResult.Fail($"Failed to list directory: {ex.Message}"));
        }
    }

    private SkillResult GetEnvironment(string arguments)
    {
        ...
        return SkillResult.Ok(ToJson(new { variables = envVars, count = envVars.Count }));
    }

    private async Task<SkillResult> WhichAsync(string arguments, CancellationToken ct)
    {
