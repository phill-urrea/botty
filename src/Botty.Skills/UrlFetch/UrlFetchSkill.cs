...
namespace Botty.Skills.UrlFetch;

/// <summary>
/// Skill for fetching and extracting readable content from URLs.
/// </summary>
public class UrlFetchSkill : BaseSkill
{
    private readonly IHttpClientFactory _httpClientFactory;
    private int _maxContentLength = 50000;
    private int _timeoutSeconds = 30;

    public UrlFetchSkill(
        IHttpClientFactory httpClientFactory,
        ILogger<UrlFetchSkill> logger)
        : base(logger)
    {
    ...
    protected override async Task<SkillResult> OnExecuteAsync(SkillContext context, CancellationToken ct)
    {
        ...
        {
            return SkillResult.Fail("Missing or invalid arguments: 'url' is required.");
        }

        if (!Uri.TryCreate(args.Url.Trim(), UriKind.Absolute, out var uri) ||
            (uri.Scheme != Uri.UriSchemeHttps && uri.Scheme != Uri.UriSchemeHttp))
        {
            return SkillResult.Fail("Invalid URL. Only absolute http/https URLs are supported.");
        }
        ...
        catch (HttpRequestException ex)
        {
            ...
            return SkillResult.Fail($"Request failed: {ex.Message}");
        }
        ...
        {
            return SkillResult.Fail($"Page returned status code {(int)response.StatusCode}.");
        }
