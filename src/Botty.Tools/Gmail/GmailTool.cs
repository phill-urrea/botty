...
namespace Botty.Tools.Gmail;

/// <summary>
/// Tool for Gmail integration supporting multiple accounts.
/// </summary>
public class GmailTool : BaseTool
{
    ...
    public GmailTool(
        ...
        ILogger<GmailTool> logger) : base(logger)
    {
    ...
    protected override async Task OnInitializeAsync(CancellationToken ct)
    {
        ...
        {
            Logger.LogWarning("Gmail tool: OAuth credentials not configured");
            return;
        }
        ...
        {
            Logger.LogWarning("Gmail tool: No linked or legacy accounts configured");
            return;
        }
        ...
        try
        {
            ...
            if (accounts.Count == 0)
            {
                Logger.LogWarning("Gmail tool: Accounts config parsed but contains no valid accounts");
                return;
            }
    ...
    private async Task<ToolResult> ListMessagesAsync(string arguments, CancellationToken ct)
    {
        ...
        if (service == null) return ToolResult.Fail("No Gmail account configured or specified");
        ...
        {
            return ToolResult.Ok(ToJson(new { messages = Array.Empty<object>(), count = 0 }));
        }
        ...
        return ToolResult.Ok(ToJson(new { messages, count = messages.Count }));
    }

    private async Task<ToolResult> GetMessageAsync(string arguments, CancellationToken ct)
    {
        var args = ParseArguments<GetMessageArgs>(arguments);
        if (args == null || string.IsNullOrEmpty(args.MessageId))
            return ToolResult.Fail("Invalid arguments: messageId required");
    ...
    private async Task<ToolResult> SendMessageAsync(string arguments, CancellationToken ct)
    {
        ...
        if (args == null || string.IsNullOrEmpty(args.To) || string.IsNullOrEmpty(args.Subject))
            return ToolResult.Fail("Invalid arguments: to and subject required");
